...
import { FAQ_CATEGORIES_EN } from "./faq-content.en";
import { FAQ_CATEGORIES_ES } from "./faq-content.es";
...
// Same content as apps/web/src/app/(app)/ajuda/faq-content.ts — kept as a
// separate copy (not shared-types) since it's plain display content, not
// validation logic that needs to stay type-locked across platforms.
export const FAQ_CATEGORIES: FaqCategory[] = [
  {
    ...
    items: [
      ...
      {
        question: "E se eu bater o ponto sem internet?",
        answer:
    ...
    items: [
      {
        ...
        answer:
          'Na tela Perfil, toque no lápis sobre a foto e escolha "Tirar foto", "Galeria" ou "Remover foto".',
      },
      {
        question: "Como troco minha senha?",
        answer: 'Na tela Perfil, toque em "Alterar senha" e informe a senha atual e a nova.',
      },
      {
        question: "Como atualizo meus dados pessoais (endereço, telefone etc.)?",
        answer:
          'Na tela Perfil, toque em "Meu Perfil". Você edita RG, data de nascimento, estado civil, telefone e endereço — nome, cargo e salário continuam exclusivos de RH/gestor.',
      },
    ],
  },
  {
...
export function getFaqCategories(locale: Locale): FaqCategory[] {
  if (locale === "en") return FAQ_CATEGORIES_EN;
  if (locale === "es") return FAQ_CATEGORIES_ES;
